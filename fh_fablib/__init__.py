...
def dev(ctx, host="127.0.0.1", port=8000, run_with=None):
    ...
    if run_with:
        jobs.append(
            f"{'uv run' if config._uv_project else '.venv/bin/python'} {run_with} manage.py runserver {backend}"
        )
    else:
        jobs.append(f"{config._manage()} runserver {backend}")
